...
var metrics = require("measured").createCollection("rampart");
    ...
    server = httpProxy.createServer(function(req, res, proxy) {
      ...
      async.parallel({
        ...
        if (!err) {
          if (cache.meta && cache.data) {
            ...
            if (maxage > 0 || cache.lock) {
              ...
              res.end(cache.data);
              cacheHit = true;
              metrics.meter(cache.lock ? "stale" : "hit").mark();
            } else {
        ...
        if (!cacheHit) {
          ...
          metrics.meter(isUpdating ? "updating" : "miss").mark();
        }
      });
    }).listen(options.port);
          ...
          if (ttl > 0) {
            ...
            response.on("end", function() {
              ...
              if (data.length < 1048576) {
                ...
                metrics.meter("uncacheable-content-length").mark();
              }
            });
          } else {
            metrics.meter("uncacheable-cache-control").mark();
          }
        } else {
          metrics.meter("uncacheable-content-type").mark();
        }
      } else {
        metrics.meter("uncacheable-headers").mark();
      }
    });
  }
  return server;
